#!/bin/bash
#SBATCH --job-name=colosseum-lingbot
#SBATCH --gres=gpu:h100-47:1
#SBATCH --time=02:00:00
#SBATCH --cpus-per-task=8
#SBATCH --mem=100G
#SBATCH --output=./logs/lingbot_vla_v2/slurm-%j.out
#SBATCH --error=./logs/lingbot_vla_v2/slurm-%j.err

# ---------------------------------------------------------------------------
# NUS SoC SLURM worker for the Robbyant/lingbot-vla-v2 Colosseum policy.
#
...
ENV_NAME="venvs/lingbot"
HOME_PATH="/home/n/ntasang"
PROJECT_PATH="${HOME_PATH}/projects/robocolosseum-host"

cd "${PROJECT_PATH}" || exit 1

LOG_DIR="./logs/lingbot_vla_v2"
mkdir -p "${LOG_DIR}"

# Activate the model's virtualenv.
source "${HOME_PATH}/${ENV_NAME}/bin/activate"

# Hugging Face cache on persistent home storage so model downloads survive
# across jobs (SLURM_TMPDIR is wiped when the job ends). lingbot-vla-v2 is a
# gated repo -- authenticate once with `huggingface-cli login` or export
# HF_TOKEN before submitting.
...
# Example: `ENABLE_ACTION=1 sbatch slurm/lingbot_vla_v2.sh` for a real session.
ACTION_ARGS=()
...
python scripts/run_policy.py \
    --policy lingbot_vla_v2 \
    --config configs/lingbot_vla_v2.yaml \
